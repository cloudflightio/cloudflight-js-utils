...
describe('SubConcurrencyPool', () => {
    describe('when creating sub pool', () => {
        it('given negative number then throw error', () => {
            ...
            expect(() => subConcurrencyPoolFrom(-1, parentPool)).toThrow(InvalidConcurrencyPoolSizeException);
        });

        it('given 0 then throw error', () => {
            const parentPool = concurrencyPoolOfSize(1);

            expect(() => subConcurrencyPoolFrom(0, parentPool)).toThrow(InvalidConcurrencyPoolSizeException);
        });
