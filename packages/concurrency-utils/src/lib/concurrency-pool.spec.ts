import {concurrencyPoolOfSize, InvalidConcurrencyPoolSizeException} from './concurrency-pool';
...
describe('ConcurrencyPool', () => {
    describe('when creating pool', () => {
        it('given negative number then throw error', () => {
            expect(() => concurrencyPoolOfSize(-1)).toThrow(InvalidConcurrencyPoolSizeException);
        });

        it('given 0 then throw error', () => {
            expect(() => concurrencyPoolOfSize(0)).toThrow(InvalidConcurrencyPoolSizeException);
        });
